...
def registered_menu(user_id=None):
    # Games is intentionally first. Telegram also lets the user collapse this keyboard.
    keyboard = [
        [KeyboardButton("🎮 Games")],
        [KeyboardButton("💰 Balance"), KeyboardButton("👤 Profile")],
        [KeyboardButton("💳 Deposit"), KeyboardButton("💸 Withdraw")],
        [KeyboardButton("👥 Invite"), KeyboardButton("📜 Transactions")],
        [KeyboardButton("⏳ Pending Requests"), KeyboardButton("📖 Game Rules")],
        [KeyboardButton("🆘 Support")],
    ]
    if user_id is not None and is_admin(user_id):
        keyboard.append([KeyboardButton("👑 Open Admin")])
    # Use the same visible player menu style as the working admin menu.
...
def admin_menu(user_id=None):
    # Admin mode shows only administrator functions.
    # The normal player menu is restored when admin mode is exited.
    keyboard = []
...
def wallet_cancel_reply_keyboard():
    """Reply keyboard shown while a player is in an active wallet process."""
    return ReplyKeyboardMarkup(
...
def wallet_bank_reply_keyboard(banks):
    """Show the available demo banks directly in the Telegram menu."""
    rows = [
        [KeyboardButton(f"🏦 {bank['bank_name']}")]
        for bank in banks
    ]
...
def wallet_input_reply_keyboard():
    """Show only Cancel while the player must type a value in chat."""
    return ReplyKeyboardMarkup(
        [[KeyboardButton("❌ Cancel")]],
        resize_keyboard=True,
        is_persistent=True,
...
def admin_settings_menu(user_id=None):
    ...
    if user_id is None or can_admin(user_id, "settings"):
        ...
        keyboard.append([KeyboardButton("💳 Deposit Limits")])
...
def bingo_admin_menu():
    return ReplyKeyboardMarkup(
        [
            ...
            [KeyboardButton("🎯 Winning Patterns")],
            [KeyboardButton("↩️ Admin Menu")],
        ],
        resize_keyboard=True,
        is_persistent=True,
...
async def start(
    ...
    if is_admin(user.id):
        await update.message.reply_text(
            ...
            "Your administrator dashboard is ready.",
...
# ============================================================
# BINGO WINNING PATTERNS / GAME RULES
# ============================================================


def format_bingo_patterns(include_disabled=True):
    patterns = get_all_bingo_patterns() if include_disabled else get_enabled_bingo_patterns()
    if not patterns:
        return "No Bingo winning patterns are configured."

    lines = []
    for pattern in patterns:
        enabled = bool(pattern["is_enabled"])
        mark = "✅" if enabled else "❌"
        lines.append(f"{mark} {pattern['pattern_name']}")
    return "\n".join(lines)


def bingo_pattern_admin_keyboard():
    patterns = get_all_bingo_patterns()
    rows = []
    for pattern in patterns:
        mark = "✅" if bool(pattern["is_enabled"]) else "❌"
        rows.append([
            InlineKeyboardButton(
                f"{mark} {pattern['pattern_name']}",
                callback_data=f"bingo_pattern_toggle:{pattern['pattern_key']}",
            )
        ])
    rows.append([InlineKeyboardButton("↩️ Bingo Game Settings", callback_data="bingo_patterns_back")])
    return InlineKeyboardMarkup(rows)


async def show_bingo_pattern_admin(update: Update):
    await update.message.reply_text(
        "🎯 BINGO WINNING PATTERNS\n\n"
        "These are the patterns that the Bingo engine will accept as winning conditions.\n\n"
        "Tap a pattern to enable or disable it.\n\n"
        "⚠️ At least one winning pattern must remain enabled.\n\n"
        + format_bingo_patterns(),
        reply_markup=bingo_pattern_admin_keyboard(),
    )


async def bingo_pattern_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    user = query.from_user

    if not can_admin(user.id, "bingo_settings"):
        await query.answer("You do not have permission to manage Bingo patterns.", show_alert=True)
        return

    if query.data == "bingo_patterns_back":
        await query.answer()
        await query.edit_message_text(
            "🎱 BINGO GAME SETTINGS\n\nManage Bingo games and winning patterns.",
        )
        return

    key = query.data.split(":", 1)[1].strip().upper()
    patterns = get_all_bingo_patterns()
    selected = next((p for p in patterns if str(p["pattern_key"]).upper() == key), None)
    if not selected:
        await query.answer("Pattern not found.", show_alert=True)
        return

    currently_enabled = bool(selected["is_enabled"])
    if currently_enabled:
        enabled_count = sum(1 for p in patterns if bool(p["is_enabled"]))
        if enabled_count <= 1:
            await query.answer("At least one winning pattern must remain enabled.", show_alert=True)
            return
        new_enabled = False
    else:
        new_enabled = True

    if not set_bingo_pattern_enabled(key, new_enabled):
        await query.answer("Could not update the pattern.", show_alert=True)
        return

    await log_admin_action(
        context,
        user.id,
        "EDIT BINGO WINNING PATTERN",
        f"{selected['pattern_name']}={'ENABLED' if new_enabled else 'DISABLED'}",
    )
    await query.answer("Pattern updated.")
    await query.edit_message_text(
        "🎯 BINGO WINNING PATTERNS\n\n"
        "Tap a pattern to enable or disable it.\n\n"
        "⚠️ At least one winning pattern must remain enabled.\n\n"
...
async def show_player_game_rules(update: Update):
    enabled = get_enabled_bingo_patterns()
    commission = get_bingo_commission_percent()
    lines = [
        "📖 BINGO GAME RULES",
        "",
        "🎯 ACTIVE WINNING PATTERNS",
        "",
    ]
    if enabled:
        ...
        lines.append("⚠️ No winning pattern is currently enabled.")

    lines.extend([
        "",
        "🏆 HOW WINNING WORKS",
        "• The server calls Bingo numbers during the game.",
        "• Your card is checked against the active patterns.",
        "• The first qualifying win for each card is recorded.",
        "• If multiple players/cards win, the prize is divided between the winners.",
        "",
        "💰 GAME POT",
        f"• Current commission: {commission}%.",
        "• The remaining prize pool is paid to the winner(s).",
        "",
        "🎫 CARDS",
        "• A player may select more than one available card.",
        "• A selected card is locked for that round.",
        "",
        "⏱ COUNTDOWN",
        "• When the required number of players joins, the countdown starts.",
        "• Players can join while the countdown is running.",
    ])
...
async def text_handler(
    ...
    if wallet_action == "DEPOSIT_BANK":
        ...
        await update.message.reply_text(
            ...
            f"🏦 Bank: {bank['bank_name']}\n"
            f"🔢 Demo Account: {bank['account_number']}\n\n"
            f"Deposit limits: {get_system_setting_float('min_deposit', 100)} - {get_system_setting_float('max_deposit', 10000)}\n\n"
            "Now enter the deposit amount.",
            reply_markup=wallet_input_reply_keyboard(),
        )
        return
    ...
    if is_admin(user.id):
        ...
        if admin_action == "SET_DEPOSIT_LIMITS":
            cleaned = text.replace(",", " ").split()
            if len(cleaned) != 2:
                await update.message.reply_text(
            ...
            try:
                ...
                await update.message.reply_text(
                    "❌ Invalid limits. Maximum must be greater than or equal to minimum, and both must be positive.",
                    reply_markup=admin_input_menu(),
                )
                return

            set_system_setting("min_deposit", minimum)
            set_system_setting("max_deposit", maximum)
            await log_admin_action(context, user.id, "EDIT DEPOSIT LIMITS", f"Minimum={minimum}; Maximum={maximum}")
            context.user_data.pop("admin_action", None)
            await update.message.reply_text(
    ...
    if is_admin(user.id):
        ...
        if text == "💳 Deposit Limits" and can_admin(user.id, "settings"):
            minimum = get_system_setting_float("min_deposit", 100)
            maximum = get_system_setting_float("max_deposit", 10000)
            context.user_data["admin_action"] = "SET_DEPOSIT_LIMITS"
            await update.message.reply_text(
                "💳 DEPOSIT LIMITS\n\n"
        ...
        if text == "🎱 Bingo Game Settings" and can_admin(user.id, "bingo_rooms"):
            context.user_data.pop("admin_action", None)
            await update.message.reply_text(
                "🎱 BINGO GAME SETTINGS\n\nManage bet amounts, available cartels, and winning patterns.",
                reply_markup=bingo_admin_menu(),
            )
            return

        if text == "🎯 Winning Patterns" and can_admin(user.id, "bingo_settings"):
            context.user_data.pop("admin_action", None)
            await show_bingo_pattern_admin(update)
            return
    ...
    if wallet_action == "DEPOSIT_AMOUNT":
        ...
        minimum_deposit = get_system_setting_float("min_deposit", 100)
        maximum_deposit = get_system_setting_float("max_deposit", 10000)
        if amount < minimum_deposit or amount > maximum_deposit:
            await update.message.reply_text(
                "❌ Deposit amount is outside the allowed limits.\n\n"
    ...
    if text == "⏳ Pending Requests":
        ...
        return
    if text == "📖 Game Rules":
        await show_player_game_rules(update)
        return
    if text == "🆘 Support":
...
async def post_init(
    ...
    # Telegram's native Menu button opens the Bingo Mini App directly.
    # The reply-keyboard hide/show behavior is otherwise unchanged.
    await application.bot.set_my_commands([
        BotCommand("menu", "Show the bot menu"),
    ])

    await application.bot.set_chat_menu_button(
        menu_button=MenuButtonWebApp(
            text="Play",
            web_app=WebAppInfo(url="https://finbase18-lab.github.io/Bingo_frontend"),
        )
    )
...
async def show_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user and get_user_by_telegram_id(user.id):
        await update.message.reply_text(
            "☰ Use Telegram's Menu button to open the bot menu.",
...
def main():
    ...
    ensure_system_setting("min_deposit", 100)
    ensure_system_setting("max_deposit", 10000)
    ...
    application.add_handler(
        CallbackQueryHandler(
            bingo_pattern_callback,
            pattern=r"^bingo_pattern_"
        )
    )
