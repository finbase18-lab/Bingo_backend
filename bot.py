...
def registered_menu(user_id=None):
    """Persistent player reply keyboard. Telegram provides the keyboard
    hide/show icon automatically beside the message box."""
    keyboard = [
        ...
        [KeyboardButton("⏳ Pending Requests"), KeyboardButton("🆘 Support")],
    ]
...
def admin_menu(user_id=None):
    ...
    if user_id is None or can_admin(user_id, "dashboard"):
        keyboard.append([KeyboardButton("📣 Broadcast to Players")])
...
def wallet_bank_reply_keyboard(banks=None):
    banks = banks or get_demo_banks()
    keyboard = [[KeyboardButton(f"🏦 {bank['bank_name']} - {bank['account_number']}")] for bank in banks]
...
def wallet_input_reply_keyboard(*args, **kwargs):
    return wallet_cancel_reply_keyboard()


def broadcast_input_menu():
    return ReplyKeyboardMarkup(
        [[KeyboardButton("❌ Cancel Broadcast")], [KeyboardButton("↩️ Admin Menu")]],
        resize_keyboard=True,
        one_time_keyboard=False,
        is_persistent=True,
    )
...
def find_bank_from_menu_text(text):
    ...
    for bank in get_demo_banks():
        if text in (bank["bank_name"], f"{bank['bank_name']} - {bank['account_number']}"):
            return bank
    return None
...
async def start_deposit(
    ...
    if not banks:
        await update.message.reply_text("❌ No demo banks are available.", reply_markup=registered_menu(update.effective_user.id))
        return
    context.user_data["wallet_action"] = "DEPOSIT_BANK"
    for key in ("deposit_bank_id", "deposit_amount", "deposit_transaction_id"):
        context.user_data.pop(key, None)
    await update.message.reply_text(
        "💳 DEMO DEPOSIT\n\nChoose the demo bank from the menu below.\n\n❌ Cancel is also available while this process is active.",
...
async def start_withdraw(
    ...
    if not user:
        await update.message.reply_text("❌ Account not found.", reply_markup=registered_menu(update.effective_user.id))
        return
    withdrawable = float(user["withdrawable_balance"] or 0)
    ...
    if withdrawable <= 0:
        await update.message.reply_text("💸 WITHDRAW\n\nYou currently have no withdrawable balance.\n\nOnly game winnings can be withdrawn.", reply_markup=registered_menu(update.effective_user.id))
        return
    if float(user["total_withdrawn"] or 0) <= 0 and float(user["total_deposited"] or 0) < first_required:
        still_needed = max(0, first_required - float(user["total_deposited"] or 0))
        await update.message.reply_text(
            "💸 FIRST WITHDRAWAL REQUIREMENT\n\n"
            f"Before your first withdrawal, you must have at least {first_required} in approved demo deposits.\n\n"
            f"✅ Approved deposits: {float(user['total_deposited'] or 0)}\n"
    ...
    for key in ("withdraw_bank_id", "withdraw_amount", "withdraw_account_number", "withdraw_account_name"):
        context.user_data.pop(key, None)
    await update.message.reply_text(
        "💸 DEMO WITHDRAWAL\n\n"
...
async def broadcast_to_players(context, text="", message=None):
    """Send an admin announcement to every registered, non-blocked player."""
    ...
    for chat_id in player_ids:
        try:
            if message is None:
                ...
                await context.bot.send_photo(chat_id=chat_id, photo=message.photo[-1].file_id, caption=message.caption or None)
            elif message.animation:
                await context.bot.send_animation(chat_id=chat_id, animation=message.animation.file_id, caption=message.caption or None)
            elif message.video:
                await context.bot.send_video(chat_id=chat_id, video=message.video.file_id, caption=message.caption or None)
            else:
                continue
            sent += 1
    ...
    logger.info("Broadcast complete: sent=%s failed=%s total=%s", sent, failed, len(player_ids))
    return sent, failed, len(player_ids)


async def broadcast_media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    if not user or not is_admin(user.id) or context.user_data.get("broadcast_action") != "WAITING":
        return
    sent, failed, total = await broadcast_to_players(context, message=update.message)
...
async def text_handler(
    ...
    # --------------------------------------------------------
    if is_admin(user.id) and context.user_data.get("broadcast_action") == "WAITING":
        if text in ("❌ Cancel Broadcast", "↩️ Admin Menu"):
            context.user_data.pop("broadcast_action", None)
            await update.message.reply_text("📣 Broadcast cancelled.", reply_markup=admin_menu(user.id))
            return
        await broadcast_to_players(context, text=text)
        context.user_data.pop("broadcast_action", None)
        await update.message.reply_text("📣 Broadcast finished.", reply_markup=admin_menu(user.id))
        return
    ...
    # --------------------------------------------------------
    if is_admin(user.id):
        if text == "📣 Broadcast to Players" and can_admin(user.id, "dashboard"):
            context.user_data["broadcast_action"] = "WAITING"
            await update.message.reply_text(
                "📣 BROADCAST TO ALL PLAYERS\n\nSend one of these next:\n• Text message\n• Photo with optional caption\n• GIF/animation with optional caption\n• Video with optional caption\n\nRegistered non-blocked players will receive it.",
                reply_markup=broadcast_input_menu(),
            )
            return
...
async def post_init(
    ...
    # Keep Telegram's own menu behavior untouched.
    # The player controls the reply keyboard with Telegram's keyboard
    # show/hide icon beside the message input field.
    try:
        ...
        try:
            ...
            logger.info("Telegram menu button configured as Play Mini App button.")
        except Exception as e:
            logger.warning("Could not configure Play menu button: %s", e)
    except Exception as e:
        logger.warning("Could not configure Telegram commands: %s", e)
