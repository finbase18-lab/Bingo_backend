...
def registered_menu(user_id=None):
    """Main player menu in Telegram ReplyKeyboard."""
    keyboard = [
        [KeyboardButton("🎮 Game"), KeyboardButton("💰 Balance")],
        [KeyboardButton("👤 Profile"), KeyboardButton("💳 Deposit")],
        [KeyboardButton("💸 Withdraw"), KeyboardButton("👥 Invite")],
        [KeyboardButton("📜 Transactions"), KeyboardButton("⏳ Pending Requests")],
...
def admin_menu(user_id=None):
    ...
    if user_id is None or can_admin(user_id, "players"):
        keyboard.append([KeyboardButton("👥 Players")])
    if user_id is None or can_admin(user_id, "broadcast"):
        keyboard.append([KeyboardButton("📢 Broadcast")])
    if user_id is None or can_admin(user_id, "dashboard"):
...
ADMIN_GRANT_DESCRIPTIONS = {
    ...
    "players": "View, edit, suspend, or delete players",
    "broadcast": "Send text, photo, GIF, or video broadcasts",
}
...
def wallet_bank_reply_keyboard(banks=None):
    ...
    keyboard = [[KeyboardButton(f"🏦 {b['bank_name']} • {b['account_number']}")] for b in banks]
...
async def text_handler(
    ...
    # --------------------------------------------------------
    # MAIN MENU MUST ALWAYS WIN OVER STALE WALLET STATE
    # --------------------------------------------------------
    # If a previous deposit/withdrawal step was interrupted, Telegram may still
    # deliver a normal menu button. Handle those buttons first so, for example,
    # Profile can never be interpreted as a withdrawal step.
    main_menu_texts = {
        "💰 Balance", "👤 Profile", "🎮 Game", "💳 Deposit",
        "💸 Withdraw", "👥 Invite", "📜 Transactions",
        "⏳ Pending Requests", "🆘 Support", "🛡️ Admin",
    }
    if text in main_menu_texts:
        for key in (
            "wallet_action", "deposit_bank_id", "deposit_amount",
            "deposit_transaction_id", "withdraw_bank_id",
            "withdraw_amount", "withdraw_account_number",
            "withdraw_account_name",
        ):
            context.user_data.pop(key, None)
        ...
        if text == "🆘 Support":
            ...
            return
        if text == "🛡️ Admin" and is_admin(user.id):
            await update.message.reply_text(
    ...
    if is_admin(user.id):
        ...
        if admin_action == "BROADCAST_TEXT":
            text_to_send = text.strip()
            if not text_to_send:
                await update.message.reply_text("❌ Broadcast message cannot be empty.", reply_markup=admin_input_menu())
                return
            sent = failed = 0
            for player_id in get_all_player_telegram_ids():
                try:
                    await context.bot.send_message(chat_id=int(player_id), text=f"📢 ANNOUNCEMENT\n\n{text_to_send}")
                    sent += 1
                except Exception:
                    failed += 1
            context.user_data.pop("admin_action", None)
            await update.message.reply_text(f"✅ Broadcast completed.\n\nSent: {sent}\nFailed: {failed}", reply_markup=admin_menu(user.id))
            return
    ...
    if is_admin(user.id):
        ...
        if text == "👥 Players" and can_admin(user.id, "players"):
            players = get_all_users_for_admin()
            lines = ["👥 PLAYERS", ""]
            for p in players[:50]:
                name = " ".join(filter(None, [p["first_name"], p["last_name"]])) or "Player"
                state = "🔴 SUSPENDED" if int(p["is_blocked"] or 0) else "🟢 ACTIVE"
                lines.append(f"{state} · {name} · {p['telegram_user_id']} · Referrals {p['referral_count']}")
            await update.message.reply_text("\n".join(lines) if len(lines)>2 else "👥 PLAYERS\n\nNo players.", reply_markup=admin_menu(user.id))
            return

        if text == "📢 Broadcast" and can_admin(user.id, "broadcast"):
            context.user_data["admin_action"] = "BROADCAST_TEXT"
            await update.message.reply_text("📢 BROADCAST\n\nEnter the message to send to all players. For photo/GIF/video, use Admin → Broadcast in the Mini App.", reply_markup=admin_input_menu())
            return
...
async def post_init(
    ...
    try:
        ...
        try:
            ensure_system_setting("bot_username", BOT_USERNAME or "")
        except Exception:
            pass
    ...
    # Telegram's blue 3-dash menu is reserved for the Mini App Play button.
    # Player actions remain in the ReplyKeyboard.
    try:
        ...
        logger.info("Telegram Menu button configured as Play Mini App.")
    except Exception as e:
        logger.warning("Could not configure Telegram Play menu button: %s", e)
...
async def show_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user and get_user_by_telegram_id(user.id):
        await update.message.reply_text(
            "🎮 Tap the blue Play menu button beside the message box to open Bingo."
        )
    else:
...
_BOT_INSTANCE_LOCK_CONNECTION = None

def acquire_bot_instance_lock():
    """Allow only one polling bot process for this PostgreSQL database."""
    global _BOT_INSTANCE_LOCK_CONNECTION
    if not USE_POSTGRES:
        return True
    try:
        connection = get_connection()
        lock_key = int(os.getenv("BOT_INSTANCE_LOCK_KEY", "47709997"))
        ...
        if not bool(row["locked"]):
            connection.close()
            logger.error("Another Bingo bot instance is already polling Telegram. This instance will not start polling.")
            return False
        _BOT_INSTANCE_LOCK_CONNECTION = connection
        logger.info("Telegram bot instance lock acquired.")
        return True
    except Exception as exc:
        logger.error("Could not acquire bot instance lock: %s", exc)
        return False


if __name__ == "__main__":
    if acquire_bot_instance_lock():
        main()
    else:
        print("Bot polling was not started because another instance owns the Telegram polling lock.")
