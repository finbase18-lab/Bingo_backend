...
from database import (
    ...
    get_bingo_elapsed_seconds,
    set_bingo_winner_started_at,
    start_bingo_playing,
...
def user_payload(account):
    ...
    return {
        ...
        "referral_code": account.get("referral_code") if hasattr(account, "get") else account["referral_code"],
    }
    ...
    with engine_lock:
        ...
        if status == "PLAYING":
            ...
            if called_count == 0:
                ...
                elapsed = get_bingo_elapsed_seconds(playing_started)
            ...
            # Keep the winner state visible for every player before moving to
            # RESULT/FINISHED. Repair older WINNER rounds created before the
            # winner_started_at PostgreSQL fix so they cannot freeze forever.
            winner_started_at = row["winner_started_at"]
            if not winner_started_at:
                set_bingo_winner_started_at(round_id)
                row = get_bingo_round(round_id)
                winner_started_at = row["winner_started_at"] if row else None
            if winner_started_at:
                elapsed = get_bingo_elapsed_seconds(winner_started_at)
                if float(elapsed or 0) >= 10:
...
def admin_players_api(search: str = "", limit: int = 200, x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data, "players")
    limit = max(1, min(int(limit or 200), 500))
    return {"success": True, "players": [row_to_dict(r) for r in get_all_users_for_admin(search, limit)]}
...
@app.patch("/api/admin/players/{telegram_user_id}")
def admin_update_player(telegram_user_id: int, body: AdminPlayerUpdateRequest, x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data, "players")
    player = get_user_by_telegram_id(telegram_user_id)
    if not player:
        raise HTTPException(status_code=404, detail="Player not found")
    if body.is_blocked is not None:
        player, status = set_user_blocked(telegram_user_id, body.is_blocked)
        if status != "updated":
            raise HTTPException(status_code=404, detail=status)
    # Balance edits use the existing transaction-safe adjustment functions.
    if body.main_balance is not None:
        delta = float(body.main_balance) - float(player["main_balance"] or 0)
        if abs(delta) > 0:
            player, status = adjust_demo_main_balance(telegram_user_id, delta)
            if status != "adjusted": raise HTTPException(status_code=409, detail=status)
    if body.bonus_balance is not None:
        delta = float(body.bonus_balance) - float(player["bonus_balance"] or 0)
        if abs(delta) > 0:
            player, status = adjust_demo_bonus_balance(telegram_user_id, delta)
            if status != "adjusted": raise HTTPException(status_code=409, detail=status)
    if body.withdrawable_balance is not None:
        delta = float(body.withdrawable_balance) - float(player["withdrawable_balance"] or 0)
        if abs(delta) > 0:
            player, status = adjust_demo_withdrawable_balance(telegram_user_id, delta)
            if status != "adjusted": raise HTTPException(status_code=409, detail=status)
    return {"success": True, "message": "Player saved successfully", "player": user_payload(player)}


@app.delete("/api/admin/players/{telegram_user_id}")
def admin_delete_player(telegram_user_id: int, x_telegram_init_data: str | None = Header(default=None)):
    require_admin(x_telegram_init_data, "players")
    if not delete_user_by_telegram_id(telegram_user_id):
        raise HTTPException(status_code=404, detail="Player not found")
...
def profile_api(x_telegram_init_data: str | None = Header(default=None)):
    ...
    referral = get_referral_stats(int(account["telegram_user_id"]))
    return {
        "success": True,
        "profile": row_to_dict(profile),
        "referral": referral,
        "bot_username": os.getenv("TELEGRAM_BOT_USERNAME", "") or get_system_setting_text("bot_username", ""),
        "user": user_payload(account),
    }
...
def bingo_dashboard(x_telegram_init_data: str | None = Header(default=None)):
    ...
    # Advance any active round once while the dashboard is polling. This also
    # lets an old WINNER/RESULT round finish even if nobody is currently on
    # the calling page. If it finishes, immediately create the next round so
    # the dashboard changes from WATCH to SELECT CARTEL.
    initial_rows = get_bingo_dashboard_data()
    for raw in initial_rows:
        if raw.get("round_id"):
            progressed = progress_round(int(raw["round_id"]))
            if progressed and progressed["status"] == "FINISHED":
                create_bingo_round(int(raw["room_id"]))
        else:
            create_bingo_round(int(raw["room_id"]))

    # Read the fresh state after progression/new-round creation.
    rooms = [room_payload(raw) for raw in get_bingo_dashboard_data()]

    # A just-created round can be absent from the filtered dashboard query in
    # an edge case; create it once more only for rooms that still have no round.
    for room in rooms:
        if not room["round_id"]:
            created, _ = create_bingo_round(int(room["room_id"]))
            if created:
                room["round_id"] = int(created["id"])
                room["round_number"] = int(created["round_number"])
                room["status"] = created["status"]
