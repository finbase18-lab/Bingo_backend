...
def progress_round(round_id):
    """Advance a round safely when the browser polls it.

    WAITING -> COUNTDOWN -> PLAYING -> WINNER -> RESULT -> FINISHED.

    The WINNER state is intentionally returned to the browser before the
    round is finished so every player has a stable response containing the
    ...
    with engine_lock:
        ...
        if status == "PLAYING":
            ...
            if called_count == 0:
                ...
                should_call = float(elapsed or 0) >= (
                    called_count * CALL_INTERVAL_SECONDS
                )
            ...
            # Older rounds may have reached WINNER before the timestamp fix.
            # Give them a timestamp now instead of freezing forever.
            ...
            if winner_started_at:
                ...
                if float(elapsed or 0) >= WINNER_DISPLAY_SECONDS:
                    # Return RESULT first. The next poll will move RESULT to
                    # FINISHED, guaranteeing at least one stable winner state.
