...
def progress_round(round_id):
    ...
    with engine_lock:
        ...
        if status == "PLAYING":
            ...
            # RESULT/FINISHED. The database stores the time the round entered
            # WINNER, and the shared helper works with SQLite and PostgreSQL.
            # Some rounds were created by an earlier database version that
            # did not have winner_started_at.  Do not let that make the state
            # endpoint fail with a KeyError and freeze every player's screen.
            try:
                winner_started_at = row["winner_started_at"]
            except (KeyError, IndexError):
                winner_started_at = None

            # For an old WINNER round with no timer, use the oldest available
            # round timestamp.  This allows an already-completed round to
            # finish and lets the next round be created normally.
            if not winner_started_at:
                winner_started_at = row["finished_at"] or row["playing_started_at"] or row["started_at"]
