...
def bingo_round_state(round_id: int, x_telegram_init_data: str | None = Header(default=None)):
    ...
    winners = []
    if row["status"] in ("WINNER", "RESULT", "FINISHED"):
        winner_result = get_bingo_winners(round_id)
        # Older database.py versions returned (winners, status), while the
        # newer compatibility function may return the winner list directly.
        if isinstance(winner_result, tuple) and len(winner_result) == 2:
            winners, winner_status = winner_result
            if winner_status != "ok":
                winners = []
        else:
            winners = winner_result or []
